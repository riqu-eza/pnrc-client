...
const Servicesmodal = ({ event, onClose, listingemail, listingname, listingaddress }) => {
 ...
 const handleConfirmBooking = async (e) => {
  ...
  switch (event.subcategory) {
    case "Shopping Malls":
      return (
        <>
          {event.numberOfStores && (
            <p className="text-lg mb-2">
              <strong>Stores:</strong> {event.numberOfStores}
            </p>
          )}
          <div className="flex flex-wrap gap-2 mb-2">
            {event.parkingAvailability && (
              <span className="bg-gray-100 px-2 py-1 rounded-full text-sm">Parking</span>
            )}
            {event.foodCourt && (
              <span className="bg-gray-100 px-2 py-1 rounded-full text-sm">Food Court</span>
            )}
            {event.securityServices && (
              <span className="bg-gray-100 px-2 py-1 rounded-full text-sm">Security</span>
            )}
          </div>
          {event.entertainmentOptions && (
            <p className="text-lg mb-2">
              <strong>Entertainment:</strong> {event.entertainmentOptions}
            </p>
          )}
        </>
      );
      ...
      return (
        <>
          {event.specialization && (
            <p className="text-lg mb-2">
              <strong>Specializes in:</strong> {event.specialization}
            </p>
          )}
        </>
      );
    ...
    case "Markets":
      return (
        <>
          ...
          {event.operatingDays && (
            <p className="text-lg mb-2">
              <strong>Open:</strong> {event.operatingDays}
            </p>
          )}
          {event.entranceFee && (
            <p className="text-lg mb-2">
      ...
      return (
        <>
          ...
          {event.yearEstablished && (
            <p className="text-lg mb-2">
              <strong>Established:</strong> {event.yearEstablished}
            </p>
          )}
          {event.authenticityGuarantee && (
    ...
    case "Souvenir Shops":
      return (
        <>
          {event.itemsAvailable && (
            <p className="text-lg mb-2">
              <strong>Items:</strong> {event.itemsAvailable}
            </p>
          )}
          {event.pricingRange && (
            <p className="text-lg mb-2">
              <strong>Price Range:</strong> ${event.pricingRange}
            </p>
          )}
        </>
      );
    ...
    case "Outlet Stores":
      return (
        <>
          {event.brandsAvailable && (
            <p className="text-lg mb-2">
              <strong>Brands:</strong> {event.brandsAvailable}
            </p>
          )}
          {event.discountRange && (
            <p className="text-lg mb-2">
              <strong>Discounts:</strong> {event.discountRange}
            </p>
          )}
        </>
      );
    ...
    case "Speciality Stores":
      return (
        <>
          ...
          {event.uniqueServices && (
            <p className="text-lg mb-2">
              <strong>Services:</strong> {event.uniqueServices}
            </p>
          )}
        </>
      );
