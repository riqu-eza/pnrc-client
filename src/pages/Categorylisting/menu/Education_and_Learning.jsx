...
import Education_and_Learningmodal from "../modal/Education_and_Learning";

const Education_and_Learningmenu = ({ subcategories, listingemail, listingname, listingaddress }) => {
  ...
  const filteredMenuItems = selectedSubcategory
    ? subcategories.find((subcat) => subcat.subcategory === selectedSubcategory)
        ?.EducationItems || []
    : subcategories.flatMap((subcat) => subcat.EducationItems); // Show all if no subcategory is selected
  ...
  {filteredMenuItems.length > 0 ? (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {filteredMenuItems.map((item) => (
        ...
        >
          {/* Image with overlay effect - kept exactly the same */}
          ...
          <div className="p-5">
            ...
            {/* Keep the same View Details button */}
    ...
    // Keep the exact same "No items found" state
...
export default Education_and_Learningmenu;
